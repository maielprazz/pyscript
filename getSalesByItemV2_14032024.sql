CREATE DEFINER=`ismail`@`%` PROCEDURE `db_ip`.`SP_RPT_getSalesByItemV2`(
...
-- 	declare edate_lm3 DATE;
	...
	SELECT 	year(a.sales_date) `YEAR`, 
			...
			case when 
				(select YEAR(a.sales_date)*10000+MONTH(a.sales_date)*100+DAY(a.sales_date) between CAST(date_format(begindate,'%Y%m%d') as UNSIGNED) and CAST(date_format(edate_lm2,'%Y%m%d') as UNSIGNED)
